...
const ExitsPage = () => {
  ...
  const handleSerialEntry = async (e) => {
    ...
    const response = await inventoryService.getAll(
      { serialNumber: inputs.serialNumber, soloStock: true },
      [
        "id",
        "partNumber",
        "descripcion",
    ...
    if (
      response.status === "OK" &&
      !selectedIds.includes(response.data[0].id)
    ) {
  ...
  const handlePartNumberEntry = async (e) => {
    ...
    setIsLoading(true);
    const response = await inventoryService.getAll(
      { partNumber: e.target["input-part-number"].value, soloStock: true },
      [
        "id",
        "partNumber",
        "descripcion",
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    const response = await inventoryService.registerExit(
      selectedIds,
      inputs.cliente,
      inputs.factura
    );
          ...
          <form>
            ...
            <div className={styles.row}>
              ...
              <Input
                ...
                type="text"
                defaultValue={inputs.responsableCliente}
                onChange={(e) => handleInputChange(e)}
              />
              <Input
                label="nota"
          ...
          <div className={styles.modal}>
            <InventoryList
              ...
              onSelect={(pickedItems, pickedIds) =>
                handleItemsPick(pickedItems, pickedIds)
              }
